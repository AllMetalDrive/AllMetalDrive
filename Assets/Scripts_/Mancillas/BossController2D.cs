...
public class BossController2D : MonoBehaviour
{
    ...
    public float fireballBurstDelay = 0.3f;
        ...
        [Header("CONFIGURACIÓN DE PROYECTIL JEFE")]
        [SerializeField] private float bossProjectileDamage = 15f;
        [SerializeField] private float bossProjectileSpeed = 12f;
    ...
    private void Update()
    {
        ...
        if (GameManagerUpdated.Instance.CurrentState != GameManagerUpdated.GameState.Gameplay)
            return; // No permitir acciones si no estamos en estado Gameplay
    ...
    private IEnumerator FireballAttackRoutine()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (fireballPrefab != null && mouthFirePoint != null)
            {
                ...
                GameObject projObj = Instantiate(fireballPrefab, mouthFirePoint.position, Quaternion.identity);
                Projectile_ projScript = projObj.GetComponent<Projectile_>();
                if (projScript != null)
                {
                    projScript.isBossProjectile = true;
                    projScript.damage = (int)bossProjectileDamage;
                    projScript.speed = bossProjectileSpeed;
    ...
    // Se comento los metodos de mirar al jugador para evitar sobreescrbirir la escala original del sprite del Boss en la escena.
    /* 
